...
# Load taxonomy
in_tax = as.matrix(read.table(
  file.path(paste(ma.dir, 'taxonomy.csv', sep='')),
  header=TRUE, sep=",",
  row.names = 1, fill=TRUE, na.strings=c("","NA","d__","p__","c__","o__","f__","g__","s__"),
  comment.char=""))
...
metaIn = read.table(meta_file, header=TRUE, row.names = 1,sep="\t", comment.char="")


my_OTU = otu_table(in_otu, taxa_are_rows = TRUE)
my_TAX = tax_table(in_tax)
my_physeq = phyloseq(my_OTU, my_TAX, metaIn, in_tree)
