...
A stretch of B<Ns> separates the denoised C<_R1> sequence and the denoise C<_R2>.

=head2 Output

The output is a similar table after joining the reads, when possible.
If using C<--verbose> a summary will be printed to the standard error at
the end.
